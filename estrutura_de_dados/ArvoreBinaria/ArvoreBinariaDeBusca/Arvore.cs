...
using System.Threading.Tasks;
    ...
    public NoArvore<T> InserirEmBusca(NoArvore<T> atual, NoArvore<T> novoNo)
    {
        if(novoNo == null)
        {
            return novoNo;
        }
        if (atual.Info.CompareTo(novoNo.Info) == 0)
        {
            return atual;
        }
        if (atual.Info.CompareTo(novoNo.Info) > 0)
        {
            atual.Esquerda = InserirEmBusca(atual.Esquerda, novoNo);
            
        }
        if(atual.Info.CompareTo(novoNo.Info) < 0){
            atual.Direita = InserirEmBusca(atual.Direita, novoNo);
        }

        return atual;
    }

    public NoArvore<T> RemoverEmBusca(NoArvore<T> atual, NoArvore<T> NoRemover)
    {
        if(NoRemover == null)
        {
            return NoRemover;
        }
        if (atual.Info.CompareTo(NoRemover.Info) == 0)
        {
            
        }
        if (atual.Info.CompareTo(NoRemover.Info) > 0)
        {
            atual.Esquerda = RemoverEmBusca(atual.Esquerda, NoRemover);

        }
        if (atual.Info.CompareTo(NoRemover.Info) < 0)
        {
            atual.Direita = RemoverEmBusca(atual.Direita, NoRemover);
        }
